...
class Bridge extends Extension {
    ...
    async bridgeOptions(message) {
        ...
        const diff = detailedDiff(settings.get(), message.options);
        ...
        const newSettings = objectAssignDeep.noMutate(diff.added, diff.updated, diff.deleted);
        ...
        if (newSettings.hasOwnProperty('permit_join')) {
            await this.zigbee.permitJoinLegacy(newSettings.permit_join);
        }
    ...
    async permitJoin(message) {
        ...
        if (typeof message === 'object') {
            ...
            time = message.time;
            if (message.device) {
                resolvedEntity = this.zigbee.resolveEntityLegacy(message.device);
        ...
        await this.zigbee.permitJoinLegacy(value, resolvedEntity, time);
    ...
    renameEntity(entityType, message) {
        ...
        if (entity.type === 'device') {
            this.publishDevices();
            const device = this.zigbee.resolveEntity(entity.device);
            this.eventBus.emit(`deviceRenamed`, {device, homeAssisantRename, from: oldFriendlyName, to});
        } else {
            this.publishGroups();
    ...
    getEntity(type, ID) {
        const entity = this.zigbee.resolveEntityLegacy(ID);
    ...
    async publishDevices() {
        const devices = this.zigbee.getDevicesLegacy().map((device) => {
            ...
            const resolved = this.zigbee.resolveEntityLegacy(device);
            const endpoints = {};
            const scenes = [];
            for (const endpoint of device.endpoints) {     
                /* istanbul ignore else */           
                if (endpoint.meta && endpoint.meta.scenes) {
                    for (const sceneAndGroupId of Object.keys(endpoint.meta.scenes)) {
                        const [sceneId, groupId] = sceneAndGroupId.split('_');
                        /* istanbul ignore else */
                        if (parseInt(groupId, 10) === ZIGBEE_DEFAULT_GROUP) {
                            scenes.push({
    ...
    async publishGroups() {
        const groups = this.zigbee.getGroupsLegacy().map((group) => {
            const resolved = this.zigbee.resolveEntityLegacy(group);
            const scenes = [];
            const members = group.members.map((m) => {
                /* istanbul ignore else */
                if (m.meta && m.meta.scenes) {
                    for (const sceneAndGroupId of Object.keys(m.meta.scenes)) {
                        const [sceneId, groupId] = sceneAndGroupId.split('_');
                        /* istanbul ignore else */
                        if (parseInt(groupId, 10) !== ZIGBEE_DEFAULT_GROUP) {
                            scenes.push({
